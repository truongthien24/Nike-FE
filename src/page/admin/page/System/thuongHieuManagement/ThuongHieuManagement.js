...
import useGetDetailThuongHieu from "./hook/useGetDetailThuongHieu";
import useGetDataThuongHieu from "./hook/useGetDataThuongHieu";
import useDeleteThuongHieu from "./hook/useDeleteThuongHieu";
...
const ThuongHieuManagement = () => {
  ...
  // Get Data
  const { thuongHieuData, isDataLoading, fetchData, isFetching } =
    useGetDataThuongHieu("0", "0");

  const {
    thuongHieuDataDetail,
    isDataDetailLoading,
    fetchData: fetchDataDetail,
    isFetching: isFetchDetail,
  } = useGetDetailThuongHieu("0", "0", showSlice?.initData?.id);
  ...
  return (
    <>
      ...
      <PopupMain
        title={
          _.isEmpty(showSlice?.initData?.id) ? "Thêm thương hiệu" : "Sửa thương hiệu"
        }
        showSlice={showSlice}

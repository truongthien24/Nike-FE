    ...
    <>
      <div className="bg-white rounded-[10px] shadow-md shadow-gray-200 over:shadow-[#5c97bc] duration-300 hover:shadow-xl cursor-pointer flex flex-col justify-between">
        <div
          ...
          className="h-[150px] bg-contain bg-center bg-no-repeat lg:h-[180px] 2xl:h-[200px] w-full rounded-t-[10px]"
        ></div>
        <div className="flex flex-col p-[10px]">
          <h5 className="font-[500] text-[16px] md:text-[18px] text-center my-[10px]">
            {data.tenDichVu}
          </h5>
        ...
        <div className="flex flex-col items-end p-[10px]">
          <button
            className={`flex items-center mt-[10px] justify-center ${
              data?.soLuong > 0
                ? "bg-[#3790c7] hover:shadow-[#3790c7a6] hover:shadow-lg"
                : "bg-[gray] hover:shadow-lg hover:shadow-gray-400"
            } text-white py-[12px] px-[20px] rounded-[10px] duration-300`}
      ...
      {isBooking && <ModalBookingService setIsBooking={setIsBooking} />}
    </>
  );
};

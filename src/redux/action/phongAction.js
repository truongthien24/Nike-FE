...
export const layDuLieuPhongTheoParams = (data) => {
  return async (dispatch) => {
    ...
    if (
      ...
      return (dataResult = result.docs.map((doc) => ({
        ...doc.data(),
        id: doc.id,
      })));
    } else {
      // Tìm kiếm phòng bằng câu lệnh query
      const loaiPhongQuery = query(
        loaiPhongRef,
      ...
      const resulta = loaiPhongResult.docs.map((doc) => ({
        ...doc.data(),
        id: doc.id,
      }));
      ...
      if (resulta.length > 0) {
        const phongQuery = query(
          phongRef,
          where("loaiPhong", "==", resulta[0].loai)
        );
        // Giải mã
...
export const layDuLieuPhongInfo = async (id) => {
  try {
    ...
    const data = result.docs.filter((item) => item.id === id);
...
export const deleteRoom = (data) => async (dispatch) => {
  try {
    ...
    if (
      dataPhieu.docs.findIndex((doc) => doc.data().idPhong === data.id) != -1
    ) {
      dispatch(
        setLoading({
          status: "done",
